...
const std::string BASE("ch5/ex5_2.");
...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        float total = std::chrono::duration_cast<std::chrono::duration<float>>(t_now - t_start).count();
        float step = std::chrono::duration_cast<std::chrono::duration<float>>(t_now - t_last).count();
        t_last = t_now;
